...
/* coloquei NIL no lugar desse failure_make, que não sei pra que que serve */

case_element_list_expr 
             : case_element_expr  {  $$ = new_node(CASE, $1, NIL); /*failure_make("case conditions are not exhaustive", FAILURE_CASE_NOT_EXHAUSTIVE, yylineno)); */ }
...
/* NIL no lugar desses find_atom, free_node comentado */

type_value_list : type_value {  $$ = cons(NIL/*find_atom($1)*/, NIL); /*free_node($1); */ } 
                | type_value_list TOK_COMMA type_value {  $$ = cons(NIL/*find_atom($3)*/, $1); /*free_node($3); */ } 
                ;
...
module       : TOK_MODULE TOK_ATOM declarations {  parse_tree = new_node(MODULE, NIL,NIL);/*$2, $3);*/  }
             ;
